...
def main():
    ...
    def get_usb_drive():
        if sys.platform.startswith('win'):
            ...
            # Use lsblk command to list block devices and filter for USB drives
            ...
            if usb_devices:
                return usb_devices[0]  # Return the first USB device found under /media
            else:
                raise FileNotFoundError("No USB drive found.")
        else:
    ...
    def encrypt_key_with_password(usb_path, key, password):
        ...
        with open(os.path.join(usb_path, "encrypted_key.key"), "wb") as encrypted_key_file:
            key_file_path = os.path.join(usb_path, "encrypted_key.key")
            encrypted_key_file.write(salt + encrypted_key)
        
        try:
        # Set the file attributes to system and hidden
            ctypes.windll.kernel32.SetFileAttributesW(key_file_path, 2 | 4)
        except Exception as e:
            result_text.delete('1.0', 'end')
            result_text.insert(f"1.0", f"File not found on windows file system - {e}")
    ...
    def update_usb_drive():
        try:
            usb_drive_path = get_usb_drive()
            usb_label.config(text=f"USB Drive: {usb_drive_path}")
            usb_label.after(1000, update_usb_drive)  # Update every 1 second
        except FileNotFoundError:
            usb_label.config(text="USB Drive: Not Found. Please attach a USB drive.")
            usb_label.after(1000, update_usb_drive)  # Update every 1 second
    ...
    usb_label = ttk.Label(root, text="USB Drive: Not Found")
    ...
    message_label.grid(row=1, column=0, columnspan=2, pady=10, padx=10, sticky="w")
    message_text.grid(row=2, column=0, columnspan=2, pady=5, padx=10, sticky="ew")
    encrypt_message_button.grid(row=3, column=0, pady=10, padx=10, sticky="ew")
    decrypt_message_button.grid(row=3, column=1, pady=10, padx=10, sticky="ew")
    encrypt_file_button.grid(row=4, column=0, pady=10, padx=10, sticky="ew")
    decrypt_file_button.grid(row=4, column=1, pady=10, padx=10, sticky="ew")
    generate_key_button.grid(row=5, column=0, columnspan=2, pady=10, padx=10, sticky="ew")
    result_text.grid(row=6, column=0, columnspan=2, pady=10, padx=10, sticky="ew")
    ...
    update_usb_drive()  # Start updating USB drive label
    root.mainloop()
