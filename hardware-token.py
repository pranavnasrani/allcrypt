...
def file_selection_window(mode):
    ...
    encrypt_file_button = tk.Button(file_selection_window, text=f"{mode} File", command=lambda: perform_file_operation(mode, input_path_entry.get(), output_path_entry.get()))
    ...
    encrypt_file_button.grid(row=4, column=0, columnspan=3, pady=10)
...
def perform_file_operation(mode, input_path, output_path):
    try:
